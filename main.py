...
def load_data():
    default_data = {
        "rate": "",
        "memo": "※ 사용방법\n- 환율은 [실시간] 버튼으로 업데이트할 수 있어요\n- VND 입력 시 000 생략은 설정에서 켜고 끌 수 있어요\n- 계산기는 금액 입력 보조용으로 사용할 수 있어요\n- 붙여넣기는 최신 복사 내용 기준으로 동작해요\n\n간편 메모장이므로 이 안내는 지우셔도 괜찮습니다 :)\n\n여행 메모  (예시)\n1만동 = 약 550원\n5만동 = 약 2,800원\n\n오늘 할일\n야시장 가기",
        "settings": {
            "auto_update": True,
            "auto_zeros": False,
            "round_krw": False,
        ...
        try:
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                ...
                if "settings" not in data:
                    ...
                    for k in ["round_krw", "auto_font", "auto_save", "calc_preview", "korean_unit", "theme"]:
                        if k not in data["settings"]:
                            data["settings"][k] = default_data["settings"][k]
                return data
        except:
            pass
    return default_data
...
class StyledButton(ButtonBehavior, Label):
    def __init__(self, bg_color=(1, 1, 1, 1), radius=20, f_size=45, t_color=TEXT_BLACK, **kwargs):
        ...
        with self.canvas.before:
            self.shadow_dark = Color(*self.dark_color)
            self.rect_dark = RoundedRectangle(pos=self.pos, size=self.size, radius=self.radius)
            self.shadow_light = Color(*self.light_color)
            self.rect_light = RoundedRectangle(pos=self.pos, size=self.size, radius=self.radius)
    ...
    def on_press(self):
        self.rect.pos = (self.pos[0] + 3, self.pos[1] - 3)
        self.shadow_light.a = 0
        self.shadow_dark.a = 0

    def on_release(self):
        self.rect.pos = self.pos
        self.shadow_light.a = 1
        self.shadow_dark.a = 1

    def update_rect(self, *args):
        self.rect_dark.pos = (self.pos[0] + 4, self.pos[1] - 4)
        self.rect_dark.size = self.size
        self.rect_light.pos = (self.pos[0] - 4, self.pos[1] + 4)
        self.rect_light.size = self.size
        self.rect.pos = self.pos
...
class ThemePillSwitch(ButtonBehavior, Widget):
    ...
    def get_theme_color(self, t_id):
        if t_id == 1:
            return (0.15, 0.45, 0.85, 1)
        elif t_id == 2:
            return (1.0, 0.6, 0.3, 1)
        else:
    ...
    def update_ui(self, *args):
        ...
        self.track_rect.radius = [self.height / 2]
...
class PillSwitch(ButtonBehavior, Widget):
    ...
    def update_ui(self, *args):
        ...
        self.track_rect.radius = [self.height / 2]
        if self.active:
            self.thumb_circle.pos = (self.right - 42 - 5, self.y + 5)
            self.track_color.rgba = self.active_color
        else:
            self.thumb_circle.pos = (self.x + 5, self.y + 5)
            self.track_color.rgba = (0.85, 0.85, 0.85, 1)
...
class CommaTextInput(TextInput):
    ...
    def insert_text(self, substring, from_undo=False):
        if substring not in "0123456789.":
            return
        if self.k_mode and self.text.endswith('K') and self.cursor[0] == len(self.text):
            self.cursor = (len(self.text) - 1, 0)
        super().insert_text(substring, from_undo=from_undo)
        self.format_and_calc()
    ...
    def format_and_calc(self):
        ...
        try:
            ...
            if self.text != formatted:
                ...
                app.is_updating = False
                if self.k_mode:
                    self.cursor = (max(0, len(self.text) - 1), 0)
                else:
                    self.cursor = (len(self.text), 0)

            app.execute_calc(raw, self)

            if hasattr(self, 'unit_label') and app.settings.get("korean_unit", True):
...
class CardInput(BoxLayout):
    def __init__(self, label_text, flag_url, cursor_c, **kwargs):
        super().__init__(**kwargs)
        self.orientation, self.size_hint_y, self.height = 'horizontal', None, 200
        self.padding, self.spacing = [20, 10], 10

        with self.canvas.before:
            Color(*CARD_BG)
            self.rect = RoundedRectangle(pos=self.pos, size=self.size, radius=[25, ])
            Color(0.85, 0.88, 0.9, 0.5)
            self.line = Line(rounded_rectangle=(0, 0, 0, 0, 25), width=1.1)
        self.bind(pos=self.update_rect, size=self.update_rect)
        ...
        label_box.add_widget(self.flag_img)
        label_box.add_widget(self.name_label)
        self.add_widget(label_box)

        input_outer = BoxLayout(orientation='vertical', size_hint_x=0.78)
        input_container = RelativeLayout(size_hint_y=None, height=150)

        self.input = CommaTextInput(
            hint_text="",
        ...
        self.input.bind(text=lambda i, v: setattr(self.clear_btn, 'opacity', 0.4 if v else 0))
        ...
        self.input.unit_label = self.unit_label

        input_outer.add_widget(input_container)
        input_outer.add_widget(self.unit_label)
        self.add_widget(input_outer)
    ...
    def set_auto_zero_mode(self, is_active):
        ...
        if is_vnd:
            ...
            if is_active:
                ...
                self.name_label.text = "VND (K)"
            else:
                self.input.hint_text = ""
                self.name_label.text = "VND"
        else:
            self.input.k_mode = False
            self.name_label.text = "KRW"
            self.input.hint_text = ""
        if self.input.text:
            self.input.format_and_calc()
...
class RateTextInput(TextInput):
    ...
    def update_label_to_manual(self):
        ...
        app.update_label.color = MUTED_COLOR
...
class SettingsPopup(ModalView):
    def __init__(self, main_app, **kwargs):
        ...
        header.add_widget(self.settings_save_btn)
        main_layout.add_widget(header)

        with main_layout.canvas.after:
            Color(0.9, 0.9, 0.9, 1)
            Line(points=[0, Window.height - 130, Window.width, Window.height - 130], width=1)
        ...
        theme_text_box.add_widget(theme_title)
        theme_text_box.add_widget(theme_desc)

        self.theme_sw = ThemePillSwitch(main_app=self.main_app, pos_hint={'right': 0.98, 'center_y': 0.5})
        theme_row.add_widget(theme_text_box)
        theme_row.add_widget(self.theme_sw)
        list_grid.add_widget(theme_row)
        ...
        s_list = [
            ...
            ("korean_unit", "한국식 단위 표시", "50,000원 → 5만원 / 500,000동 → 50만동 으로 표시"),
        ]
        ...
        for k, t, desc in s_list:
            ...
            sw = PillSwitch(
                active_color=self.main_app.c_main,
                active=bool(self.main_app.settings.get(k, False)),
                pos_hint={'right': 0.98, 'center_y': 0.5}
            )
    ...
    def save_and_close(self, inst):
        self.is_saved = True
        for k in ["auto_update", "auto_zeros", "round_krw", "auto_font", "auto_save", "calc_preview", "korean_unit"]:
        ...
        if self.main_app.row1.input.text:
            self.main_app.row1.input.format_and_calc()
        if self.main_app.row2.input.text:
            self.main_app.row2.input.format_and_calc()
        self.dismiss()
...
class QuickRatePopup(ModalView):
    def __init__(self, current_rate, **kwargs):
        ...
        for vnd in vnd_list:
            ...
            box.add_widget(vnd_lbl)
            box.add_widget(arrow_lbl)
            box.add_widget(krw_lbl)
            row.add_widget(box)
            grid.add_widget(row)
...
class ThemeSavePopup(ModalView):
    def __init__(self, on_confirm, on_select_all, **kwargs):
        ...
        with main_layout.canvas.before:
            Color(1, 1, 1, 1)
            self.bg_rect = RoundedRectangle(pos=(0, 0), size=self.size, radius=[35])
            Color(0.85, 0.85, 0.85, 1)
            self.h_line = Line(points=[0, 110, self.width, 110], width=1)
...
class CalculatorPopup(ModalView):
    def __init__(self, main_app, target_row, **kwargs):
        ...
        btns = [
            ['7', NUM_LIGHT_GRAY], ['8', NUM_LIGHT_GRAY], ['9', NUM_LIGHT_GRAY], ['/', c_sub],
            ['4', NUM_LIGHT_GRAY], ['5', NUM_LIGHT_GRAY], ['6', NUM_LIGHT_GRAY], ['*', c_sub],
            ['1', NUM_LIGHT_GRAY], ['2', NUM_LIGHT_GRAY], ['3', NUM_LIGHT_GRAY], ['-', c_sub],
            ['C', (1, 0.88, 0.88, 1)], ['0', NUM_LIGHT_GRAY], ['BACKSPACE', NUM_LIGHT_GRAY], ['+', c_sub],
        ...
        for txt, clr in btns:
            ...
            if txt == 'FLAG':
                ...
                f_layout.add_widget(self.pop_flag_img)
                grid.add_widget(f_layout)
                continue
    ...
    def update_disp_rect(self, instance, value):
        self.disp_rect.pos, self.disp_rect.size = instance.pos, instance.size
        self.disp_line.rounded_rectangle = (instance.x, instance.y, instance.width, instance.height, 20)

    def toggle_currency(self, inst):
        self.main_app.swap()
        self.target_row = self.main_app.row1
        self.pop_flag_img.source = self.target_row.flag_img.source
    ...
    def update_converted(self):
        ...
        try:
            ...
            use_korean = self.main_app.settings.get("korean_unit", True)
            ...
            if is_vnd:
                ...
                result = int(result)
                k_unit = to_korean_unit(result)

                if use_korean and k_unit:
                    self.converted_label.text = f"≈ {result:,}원 ({k_unit}원)"
                else:
                    self.converted_label.text = f"≈ {result:,}원"
            else:
                result = int(v * r)
                ...
                if is_auto_zeros:
                    if use_korean and k_unit:
                        self.converted_label.text = f"≈ {result // 1000:,}K동 ({k_unit}동)"
                    else:
                        self.converted_label.text = f"≈ {result // 1000:,}K동"
                else:
                    if use_korean and k_unit:
                        self.converted_label.text = f"≈ {result:,}동 ({k_unit}동)"
                    else:
                        self.converted_label.text = f"≈ {result:,}동"
        except:
            self.converted_label.text = ""
    ...
    def apply(self, inst):
        if self.formula.text and "=" not in self.formula.text:
            try:
                expr = (self.formula.text + self.display.text).replace(',', '')
                res = str(eval(expr))
                res = res.split('.')[0] if '.' in res and res.split('.')[1] == '0' else res
                self.formula.text += self.display.text + " ="
                self.display.text = "{:,}".format(int(float(res)))
                self.apply_btn.text = "금액 적용하기"
                self.update_converted()
            except:
                self.display.text, self.formula.text = "Error", ""
                self.converted_label.text = ""
        else:
            val = self.display.text.replace(',', '')
            if val and (val.replace('.', '', 1).isdigit()):
                num = float(val)
                k_on = getattr(self.target_row.input, 'k_mode', False)
                raw_num = num / 1000 if k_on else num
                formatted_val = "{:,}".format(int(raw_num))
                if k_on:
                    formatted_val += "K"
                self.target_row.input.text = formatted_val
                self.main_app.execute_calc(str(raw_num), self.target_row.input)
            self.dismiss()

class ExchangeRateApp(App):
    def set_theme_colors(self, theme_id):
        if theme_id == 1:
            self.c_main, self.c_sub, self.c_muted, self.c_memo, self.c_text = (
                (0.15, 0.45, 0.85, 1),
                (0.4, 0.6, 0.85, 1),
                (0.5, 0.6, 0.75, 1),
                (0.92, 0.95, 0.98, 1),
    ...
    def update_rate_desc(self):
        ...
        if r:
            try:
                self.rate_desc_label.text = f"1원 = {float(r):.2f}동"
            except:
    ...
    def build(self):
        ...
        settings_layout.add_widget(settings_img)

        header_box.add_widget(self.title_lbl)
        header_box.add_widget(settings_layout)
    ...
    def update_ui_for_settings(self):
        ...
        if self.row1.input.text:
            self.row1.input.format_and_calc()
        if self.row2.input.text:
            self.row2.input.format_and_calc()

    def clear_all_inputs(self, inst):
        try:
            self.is_updating = True
            self.row1.input.text = ""
            self.row2.input.text = ""
            self.row1.unit_label.text = ""
            self.row2.unit_label.text = ""
        finally:
    ...
    def execute_calc(self, value, source_input):
        ...
        target_unit = self.row2.unit_label if is_row1 else self.row1.unit_label
        ...
        try:
            ...
            self.is_updating = True
            if getattr(target_input, 'k_mode', False):
                target_input.text = f"{int(display_res):,}K"
            else:
                target_input.text = f"{int(display_res):,}"

            if self.settings.get("korean_unit", True):
                unit_str = to_korean_unit(int(target_actual))
                if unit_str:
                    target_unit.text = f"{unit_str}동" if target_is_vnd else f"{unit_str}원"
                else:
                    target_unit.text = ""
            else:
                target_unit.text = ""
